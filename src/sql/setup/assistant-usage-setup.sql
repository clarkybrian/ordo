-- Script d'ajout de la table assistant_usage pour le nouveau système
-- À exécuter dans l'éditeur SQL de Supabase

-- 1. Créer la table assistant_usage
...
-- 4. Créer les politiques de sécurité (en supprimant d'abord si elles existent)
DROP POLICY IF EXISTS "Users can view own assistant usage" ON assistant_usage;
DROP POLICY IF EXISTS "Users can insert own assistant usage" ON assistant_usage;
DROP POLICY IF EXISTS "Users can update own assistant usage" ON assistant_usage;
...
-- 5. Fonction pour obtenir ou créer l'usage d'un utilisateur
...
-- 6. Fonction pour incrémenter le compteur de questions
